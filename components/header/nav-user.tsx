...
export const NavUser = ({ user, compact = false }: NavUserProps) => {
  ...
  const displayAvatar =
    user?.photo_profile && user.photo_profile.trim().length > 0
      ? formatUrl(user.photo_profile) || "/avatars/shadcn.jpg"
      : "/avatars/shadcn.jpg";
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant={"ghost"}
          size={compact ? "icon" : "lg"}
          className="text-white data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground max-w-[250px]"
        >
